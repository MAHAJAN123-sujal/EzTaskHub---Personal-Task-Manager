...
import SortByCompletedAt from '../components/SortByCompletedAt';
    ...
    return (
      <div className='relative p-6 md:p-10'>
        <div className="flex flex-col md:flex-row md:justify-between">
          ...
          <SortByCompletedAt tasks={filteredTasks} onFilter={handleSetSortedTasks}/>
        </div>
          ...
          <div className="flex flex-col gap-2">
            {sortedTasks.length>0 ?(
              sortedTasks.map((task) =>(
                <ReadOnlyTaskBox key={task._id} task={task} fetchingTasks={fetchingTasks} status='completed'/>
              ))
            ):(
              <div className="flex items-center justify-center gap-2">
                <CircleOff className='size-6'/>
                <p className='text-3xl text-gray-800'>No Completed Tasks</p>
              </div>
            )}
          </div>
        ):(
